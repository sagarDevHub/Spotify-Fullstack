...
//   const { currentSong, isPlaying, togglePlay, playNext, playPrevios } =
//     usePlayerStore();
...
const PlaybackControls = () => {
  ...
  return (
    <footer className="h-20 sm:h-24 bg-zinc-900 border-t border-zinc-800 px-4 fixed bottom-0 left-0 right-0 z-50">
      <div className="flex justify-between items-center h-full max-w-[1800px] mx-auto">
        {/* Song Info */}
        <div className="flex items-center gap-2 sm:gap-4 min-w-0 w-[40%] sm:w-[30%]">
          {currentSong && (
            <>
              <img
                src={currentSong.imageUrl}
              ...
              />
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium truncate hover:underline cursor-pointer text-white">
                  {currentSong.title}
                </div>
        ...
        <div className="flex flex-col items-center gap-2 flex-1 max-w-full sm:max-w-[45%]">
          ...
          {/* Seek Bar - Visible on all screens now */}
        ...
        {/* Volume + Extras (only on larger screens) */}
        <div className="hidden sm:flex items-center gap-4 min-w-[180px] w-[30%] justify-end">
          <Button
            size="icon"
            variant="ghost"
